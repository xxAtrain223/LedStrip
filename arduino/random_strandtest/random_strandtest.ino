...
void loop() {
	...
	if (calculateColors)
	{
		...
		//fill_rainbow(leds, NUM_LEDS, interp.Time, deltaHue);
...
void attachCommandCallbacks()
{
	...
	cmdMessenger.attach(kSetDeltaHue, cmdSetDeltaHue);
...
void uploadPattern(PyInt::InstructionSet set)
{
	...
	for (byte i = 0; i < INSTRUCTION_ARRAY_SIZE && opcode != PythonOpcode::RETURN_VALUE; i++)
	{
...
void cmdSetDeltaHue()
{
	...
	deltaHue = cmdMessenger.readBinArg<byte>();

	if (requested_ack)
		cmdMessenger.sendBinCmd(kAcknowledge, kSetDeltaHue);
}
...
void resetEeprom()
{
	interp.r_instructions[0] = { PythonOpcode::LOAD_CONST, 255 };
	interp.r_instructions[1] = { PythonOpcode::RETURN_VALUE, 0 };

	interp.g_instructions[0] = { PythonOpcode::LOAD_CONST, 255 };
	interp.g_instructions[1] = { PythonOpcode::RETURN_VALUE, 0 };

	interp.b_instructions[0] = { PythonOpcode::LOAD_CONST, 255 };
	interp.b_instructions[1] = { PythonOpcode::RETURN_VALUE, 0 };
